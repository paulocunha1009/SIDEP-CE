-- SIDEP-CE - Login do aluno tambem aceita e-mail institucional
-- Data: 08/08/2026
--
-- Extensao da migration anterior (migration_2026_08_08_aluno_matricula_vinculo.sql):
-- o aluno passa a poder digitar matricula OU e-mail institucional
-- (@aluno.ce.gov.br) cadastrados em aluno_matricula, alem do nome completo
-- (fallback, sempre funciona). Mesma logica hibrida e aditiva de antes -
-- nada quebra para quem ainda nao foi importado.
...
begin
  ...
  if v_identificador = '' then
    raise exception 'Informe seu nome completo, matricula ou e-mail institucional.';
  end if;

  select am.matricula, am.nome_completo
  into v_matricula, v_nome
  from aluno_matricula am
  where (am.matricula = v_identificador or lower(am.email_institucional) = lower(v_identificador))
    and am.ativo = true;

  if v_matricula is null then
    if array_length(regexp_split_to_array(v_identificador, '\s+'), 1) < 2 then
      raise exception 'Informe o nome completo do estudante, a matricula ou o e-mail institucional cadastrado.';
    end if;
